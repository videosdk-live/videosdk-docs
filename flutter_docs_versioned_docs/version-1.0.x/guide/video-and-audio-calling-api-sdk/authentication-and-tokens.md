...
For security, every participant that connects to room needs a access token. By substituting `apikey` and `permissions` in it.
...
jwt.sign(
  {
    apikey: API_KEY,
    permissions: ["allow_join"], // Permission to join the room
  },
  SECRET_KEY,
  {
    algorithm: "HS256",
...
- **allow_mod**: Allow participant to enable/disable other participant's mic/camera.
