...
function NotFound() {
  ...
  useEffect(() => {
    ...
    const docs = ["api-reference", "prebuilt", "flutter", "react", "android", "javascript", "ios", "react-native"];
    let isDocs = false;
    ...
    if (isDocs) {
      ...
      var newPath = "/" + currentPath.split("/")[1] + ((version.match("([0-9])+.([0-9])+.([0-9]|[a-z])+") || version.match("(v[0-9])+")) && version != versions[0] && isValidVersion ? "/" + currentPath.split("/")[2] + "/" : "/")

      //update start page path for the sdk
      newPath = newPath +
        (sdk == "api-reference"
          ? "realtime-communication/intro"
          : currentPath.split("/")[(version.match("([0-9])+.([0-9])+.([0-9]|[a-z])+") || version.match("(v[0-9])+")) ? 3 : 2] == "guide"
            ? sdk == "prebuilt"
              ? "guide/prebuilt-video-and-audio-calling/getting-started" : "guide/video-and-audio-calling-api-sdk/getting-started"
            : "api/sdk-reference/setup")
      console.log("paths", newPath);
