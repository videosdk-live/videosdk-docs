...
function RestApiDescription({ title, description }) {
  return (
...
function RestApiEndPointList({ endpoints }) {
  return (
    <div>
      ...
      <div className="pt-4 pl-4 pr-4 pb-2 bg-[#252a34] rounded-b-lg flex flex-col align-middle">
        {endpoints.map((endpoint, index) => {
          return (
            <p className="mb-2">
              <span className="text-[#72C894] text-sm font-bold max-w-min">
                {endpoint.methodType}
              </span>
...
export default function RestApiEndpointContainer({ endpointSections }) {
  return (
    <div id="tailwind">
        ...
        return (
          <div key={`${index}`} className="py-7">
            <div className="flex lg:flex-row flex-col w-full">
              <div className="lg:w-1/2 w-full self-start lg:pr-[18px] flex-grow top-10">
                <RestApiDescription
                  description={endpointSection.description}
                  title={endpointSection.title}
                />
              </div>
              <div className="lg:w-1/2 w-full lg:pt-0 pt-4 lg:pl-[18px] self-start flex-grow top-10">
                <br /> <br />
                <RestApiEndPointList endpoints={endpointSection.endpoints} />
              </div>
            </div>
            <div className="bg-[#252A34] mt-12 mb-1 h-[1px]"></div>
          </div>
        );
      })}
    </div>
