...
- **`defaultCamera`**:
  ...
  - It will accept two values : `front` or `back`
...
To know more about other properties, you can follow [our API Reference](/react-native/api/sdk-reference/meeting-provider).
...
`useParticipant` hook abstracts participant class and is responsible to provide the states and events update happening for a particular participant like webcam, mic streams status etc. To know more about the properties and events accessible by this hook, go through [our API Reference](/react-native/api/sdk-reference/use-participant/introduction).
